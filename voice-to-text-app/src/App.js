...
const RecordingScreenWrapper = () => {
  ...
  // ✅ NEW: holds the prediction result to pass down to RecordingScreen
  const [lastPrediction, setLastPrediction] = useState(null);

  const handleRecordingSaved = (prediction) => {
    // Store the prediction so RecordingScreen can display it
  ...
  useEffect(() => {
    ...
    const checkServer = async () => {
      try {
        ...
        navigate('/', { replace: true, state: { error: 'Failed to connect to the server.' } });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const totalItemsCompleted = (runsCompleted * ITEMS_TO_RECORD.length) + currentItemIndex;
  ...
  return isValidStudentId ? (
    <RecordingScreen
      ...
      onRedo={handleRedo}
      lastPrediction={lastPrediction}       // ✅ NEW prop
    />
  ) : null;
};
